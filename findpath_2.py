...
path = 'G:\竞赛学术\双智竞赛\智慧城市竞赛\成果\论文数据/time'
time_cost = 0
cost2 = 0
datas = pd.read_excel("G:\竞赛学术\双智竞赛\智慧城市竞赛\区域七"
                      ".xls")
...
def cost(sum1, sum2):
    ...
    return 1/(1+math.exp(sum1*-2+sum2*-1))
...
def findallway(x):
    ...
    main_points, pass_points = find_mainline(start_point, treepoints, pass_points, end_point, x[-2])
...
if __name__ == "__main__":
    ...
    y = list(0 for i in range(int(a)+2))
    z = []
    ...
    # 使用遗传算法求解目标函数的最小值
    # 构造遗传算法对象 ex_GA
    ex_GA = sko.GA.GA(func=findallway, n_dim=(int(a)+2), size_pop=6, max_iter=100, prob_mut=0.6,
    ...
    with open(path + r"\route_best.txt", mode= "w" ) as tf:
